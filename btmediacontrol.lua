...
local function kindleLipcPidFile(svc)
    return "/tmp/abk_lipc_" .. svc:gsub("%.", "_") .. ".pid"
end
...
function BtMediaControl._startKindleLipcEventWatch()
    ...
    for _, svc in ipairs(KINDLE_LIPC_WATCH) do
        -- Background the watcher and record its pid. Killing a wrapper shell
        -- would orphan lipc-wait-event across sessions, so stop() and the
        -- startup sweep (main.lua) both work from these pidfiles instead.
        os.execute(string.format(
            "lipc-wait-event -m %s '*' >> %s 2>/dev/null & echo $! > %s",
            svc, KINDLE_AVRCP_LOG, kindleLipcPidFile(svc)))
    end
...
function BtMediaControl._stopKindleLipcEventWatch()
    ...
    for _, svc in ipairs(KINDLE_LIPC_WATCH) do
        local pid_file = kindleLipcPidFile(svc)
...
function BtMediaControl._consumeKindleLipcEvents()
    ...
    for line in chunk:gmatch("[^\n]+") do
        ...
        if line:find("audioOutput", 1, true)
            ...
            local ev = line:lower()
            -- Frontier patterns so "paused", "playing" and "InPlayback"
            -- never read as bare pause / play commands.
...
function BtMediaControl._pollKindleAvrcp()
    ...
    if BtMediaControl._kindle_avrcp_active then
        -- LIPC events land in the log from background waiters; a 0.4 s poll
        -- keeps stem clicks responsive without busy-waiting.

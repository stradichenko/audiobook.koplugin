...
function HighlightManager:new(o)
    ...
    -- The document view the pending boxes were captured on; screen-coordinate
    -- boxes must not be stamped on a moved view.
...
--- False when the user (or auto-advance) has moved to another page or view,
...
-- to avoid id collisions across chapters (epubfmt.cpp builds the prefix
-- per spine item, lvtinydom.cpp convertId appends "_" + " " + id).
-- Confirmed via getHTMLFromXPointer dump: raw SMIL id "part0020.xhtml-s36"
-- appears in the DOM as "_doc_fragment_20_ part0020.xhtml-s36" (spine
-- index 21, 1-based, minus 1). Reuses MediaSync:_getExpectedDocFragmentIndex
-- (spine-position lookup) so the index is not duplicated in two places.
...
-- Falls back to the raw id when the spine index is unavailable (unknown
-- text_doc, no SMIL parser), so lookup never degrades below the old
-- behavior.
function HighlightManager:_resolveFragmentXPointer(doc, fragment_id, text_doc)
    ...
    local crengine_id = self:_toCrengineId(fragment_id, text_doc) or fragment_id
    ...
    local ms = self.plugin and self.plugin.media_sync
    local n = (ms and ms:_getExpectedDocFragmentIndex(text_doc))
        or self:_currentDocFragmentIndex(doc)
    if not n then return nil end
    ...
    for _, body in ipairs(bodies) do
        hit = try_xp(string.format("/body/DocFragment[%d]/%s/id('%s')", n, body, crengine_id))
...
function HighlightManager:_fragmentTextRange(doc, fragment_id, sentence)
    ...
    -- stub. getTextFromXPointer is just node->getText8() at the element
    -- start, which returns only the first word/node at a collapsed xpointer
    -- and silently produced a too-short `want` target for ANY sentence
    -- where the stub differs from the real text -- not just one-word ("I")
    -- sentences, though those are the most visible case.
    local node_text = ""
    ...
    if next_id and doc.getNormalizedXPointer then
        pcall(function()
            ...
            -- (crengine renamed ids), so probe its namespaced form too;
            -- _nextSmilFragmentId only returns ids in the same text_doc,
            -- making the current spine index the right one to use.
            local next_crengine = self:_toCrengineId(next_id, sentence and sentence.text_doc)
                or next_id
            local nxp = doc:getNormalizedXPointer("#" .. next_crengine)
    ...
    local xp1 = xp0
    -- Compare joined characters, not token counts: crengine's word-boundary
    -- stops don't align 1:1 with tokenizeForAlign's merged tokens (dropped-cap
    -- names, contractions, French elisions like "L'homme" that still merge
    -- here). A token-count bound failed mid-token or stopped one crengine
    -- word short of the sentence end; character prefixes cannot disagree.
    local want_joined = table.concat(want, "")
    if doc.getNextVisibleWordEnd then
        ...
        for _ = 1, 400 do
            ...
            drawn = Utils.normalizeForMatching(drawn)
            local dw_joined = table.concat(foldedWords(drawn), "")
            if dw_joined == "" then break end
            if #dw_joined > #want_joined then break end
            if dw_joined ~= want_joined:sub(1, #dw_joined) then break end
            xp1 = nxt
            last = nxt
            if #dw_joined >= #want_joined then break end
        end
